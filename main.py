...
null_year_ref_ids = [
    pid for pid in all_ref_ids if reference_metadata[pid]['year'] is None
]
...
def get_embedding(paper, embedding_map=None):
    ...
    if embedding_map and 'paper_id' in paper and paper['paper_id'] is not None and paper['paper_id'] in embedding_map:
...
def get_recommendations(model, paper_obj, knn_tree, embedding_map, metadata, all_pids, top_k, knn_k):
    model.eval()
...
def recommend():
    ...
    recommendations_ids, scores = get_recommendations(model, paper_obj, knn_tree, embedding_map, metadata, all_pids, top_k=num_papers, knn_k=hparams['knn_k'])
